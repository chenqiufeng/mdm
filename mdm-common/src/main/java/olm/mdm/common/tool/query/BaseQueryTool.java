...
import olm.mdm.common.tool.database.ColumnInfo;
import olm.mdm.common.tool.database.DasColumn;
import olm.mdm.common.tool.database.TableInfo;
import olm.mdm.common.tool.meta.DatabaseInterface;
import olm.mdm.common.tool.meta.DatabaseMetaFactory;
import olm.mdm.common.utils.JdbcUtils;
...
public abstract class BaseQueryTool implements QueryToolInterface {
     ...
     */
    BaseQueryTool(DatasourceEntity jdbcDatasource) throws SQLException {
        if (LocalCacheUtil.get(jdbcDatasource.getDatasourceName()) == null) {
            getDataSource(jdbcDatasource);
        } else {
            this.connection = (Connection) LocalCacheUtil.get(jdbcDatasource.getDatasourceName());
            if (!this.connection.isValid(500)) {
                LocalCacheUtil.remove(jdbcDatasource.getDatasourceName());
                getDataSource(jdbcDatasource);
            }
        }
        sqlBuilder = DatabaseMetaFactory.getByDbType(jdbcDatasource.getDatasource());
        currentSchema = getSchema(jdbcDatasource.getJdbcUsername());
        currentDatabase = jdbcDatasource.getDatasource();
        LocalCacheUtil.set(jdbcDatasource.getDatasourceName(), this.connection, 4 * 60 * 60 * 1000);
    }

    private void getDataSource(DatasourceEntity jdbcDatasource) throws SQLException {
        String userName = jdbcDatasource.getJdbcUsername();

        //这里默认使用 hikari 数据源
        HikariDataSource dataSource = new HikariDataSource();
        dataSource.setUsername(jdbcDatasource.getJdbcUsername());
        dataSource.setPassword(jdbcDatasource.getJdbcPassword());
        dataSource.setJdbcUrl(jdbcDatasource.getJdbcUrl());
        dataSource.setDriverClassName(jdbcDatasource.getJdbcDriverClass());
