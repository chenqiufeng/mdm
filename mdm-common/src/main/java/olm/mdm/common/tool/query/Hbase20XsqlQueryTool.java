...
public class Hbase20XsqlQueryTool extends BaseQueryTool implements QueryToolInterface {
     ...
     */
    public Hbase20XsqlQueryTool(DatasourceEntity jdbcDatasource) throws SQLException {
        super(jdbcDatasource);

        if (LocalCacheUtil.get(jdbcDatasource.getDatasourceName()) == null) {
            getDataSource(jdbcDatasource);
        } else {
            conn = (Connection) LocalCacheUtil.get(jdbcDatasource.getDatasourceName());
            if (conn == null) {
                LocalCacheUtil.remove(jdbcDatasource.getDatasourceName());
            }
        }
        LocalCacheUtil.set(jdbcDatasource.getDatasourceName(), conn, 4 * 60 * 60 * 1000);
    ...
    private void getDataSource(DatasourceEntity jdbcDatasource) throws SQLException {
        conn = DriverManager.getConnection(jdbcDatasource.getJdbcUrl());
