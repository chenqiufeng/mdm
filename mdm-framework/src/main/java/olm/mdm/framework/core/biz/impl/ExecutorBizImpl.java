package olm.mdm.framework.core.biz.impl;
...
import olm.mdm.framework.core.glue.GlueFactory;
import olm.mdm.framework.core.glue.GlueTypeEnum;
import olm.mdm.framework.core.handler.IJobHandler;
import olm.mdm.framework.core.handler.impl.GlueJobHandler;
import olm.mdm.framework.core.handler.impl.ScriptJobHandler;
import olm.mdm.framework.core.log.JobFileAppender;
import olm.mdm.framework.core.thread.JobThread;
...
public class ExecutorBizImpl implements ExecutorBiz {
    ...
    public ReturnT<String> run(TriggerParam triggerParam) {
        ...
        JobThread jobThread = JobExecutor.loadJobThread((int) triggerParam.getJobId());
        ...
        if (GlueTypeEnum.BEAN == glueTypeEnum) {
            ...
            // valid handler
            if (jobHandler == null) {
                jobHandler = new ScriptJobHandler((int) triggerParam.getJobId(), triggerParam.getGlueUpdatetime(), triggerParam.getGlueSource(), GlueTypeEnum.match(triggerParam.getGlueType()));
            }
        } else {
        ...
        if (jobThread == null) {
            jobThread = JobExecutor.registJobThread((int) triggerParam.getJobId(), jobHandler, removeOldReason);
        }
