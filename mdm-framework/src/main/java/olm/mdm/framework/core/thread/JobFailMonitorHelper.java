...
public class JobFailMonitorHelper {
					...
					try {
						...
						if (failLogIds!=null && !failLogIds.isEmpty()) {
							for (long failLogId: failLogIds) {
								...
								JobInfo info = JobAdminConfig.getAdminConfig().getJobInfoMapper().selectJobInfoById(log.getJobId());
	...
	private boolean failAlarm(JobInfo info, JobLog jobLog){
		...
		if (info!=null && info.getAlarmEmail()!=null && info.getAlarmEmail().trim().length()>0) {
			...
			// email info
			JobGroup group = JobAdminConfig.getAdminConfig().getJobGroupMapper().load(info.getJobGroup());
