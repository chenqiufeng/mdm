package olm.mdm.framework.core.trigger;
...
import olm.mdm.framework.core.enums.IncrementTypeEnum;
import olm.mdm.framework.core.glue.GlueTypeEnum;
import olm.mdm.framework.core.route.ExecutorRouteStrategyEnum;
...
import olm.mdm.rpc.util.IpUtil;
import olm.mdm.rpc.util.ThrowableUtil;
...
public class JobTrigger {
     ...
     */
    public static void trigger(long jobId, TriggerTypeEnum triggerType, long failRetryCount, String executorShardingParam, String executorParam) {
        JobInfo jobInfo = JobAdminConfig.getAdminConfig().getJobInfoMapper().selectJobInfoById(jobId);
        if (jobInfo == null) {
...
//            String json = JSONUtils.changeJson(jobInfo.getJobJson(), JSONUtils.decrypt);
        ...
        long finalFailRetryCount = failRetryCount >= 0 ? failRetryCount : jobInfo.getExecutorFailRetryCount();
     ...
     */
    private static void processTrigger(JobGroup group, JobInfo jobInfo, long finalFailRetryCount, TriggerTypeEnum triggerType, int index, int total) {
        ...
        jobLog.setTriggerCode((long) triggerResult.getCode());
    ...
    private static long getMaxId(JobInfo jobInfo) {
        JdbcDatasource datasource = JobAdminConfig.getAdminConfig().getJdbcDatasourceMapper().selectJdbcDatasourceById(jobInfo.getDatasourceId());
